            ...
            for el in self.from_answer_link:
                for answer in el["answers"]:
                    if self.is_order_matching(audio, answer):
                        if self.run_synapse_by_name_with_order(audio, el["synapse"], order_template=answer):
                            found = True
                        else:
                            logger.debug("Neurotransmitter, the synapse has not been found : %s" % el["synapse"])
                            self.run_synapse_by_name(self.default)
